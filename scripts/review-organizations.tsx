...
const members = ['陳怡君', '王大明', '林恩典', '許以樂', '張佳恩', '黃思源'].map((name, index) => ({
  memberId: id(100 + index), displayName: name, email: 'member' + index + '@example.test',
...
const fixtureFetch: typeof fetch = async (input, init) => {
  ...
  if (url.pathname.endsWith('/me/resources')) return reply({ items: [] })
  if (url.pathname.endsWith('/me/access')) return reply({ responsibilities: [{ orgUnitId: id(1) }], roles: [], entitlements: [] })
  ...
  if (suffix === '/members') return reply({ items: members.filter(member => (member.displayName + member.email).includes(query)), page: Number(url.searchParams.get('page') || 1) })
...
window.history.replaceState(null, '', '/organizations/' + id(1) + '?locale=zh-Hant')
createRoot(document.getElementById('root')!).render(<BrowserRouter><LocaleProvider><ThemeProvider><AuthProvider api={account} operationsApi={new OperationsApi(createOperationsClient({ baseUrl: location.origin, getAccessToken: async () => 'local-review', refreshAfterUnauthorized: async () => null, fetcher: fixtureFetch }))} unitNotificationsApi={new UnitNotificationsApi({ getAccessToken: () => 'local-review', fetch: fixtureFetch })}><App /></AuthProvider></ThemeProvider></LocaleProvider></BrowserRouter>)
